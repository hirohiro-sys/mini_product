...
READMEはQiitaの以下の記事を参考に作成しました。まだ学習段階の小規模なアプリのため、ER図やインフラ構成図などは作ってません。また初めはデプロイをして誰かに使ってもらうことを考えていたのですが、セキュリティ関連の知識など不足しているものが多すぎると感じたので、renderで一度ホスティングしてから現在は公開していません。どんなアプリかのイメージを持ってもらうためにトップページの画像だけ貼っておきます。
...
開発に必要な要素を一つ一つ学習して一つのアプリという形でアウトプットしていく過程を通してソフトウェア開発の面白さを実感できた。また開発を通して自分に足りない部分がこんなにもあるんだと知った。今後は
